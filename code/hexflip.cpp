...
const int L       = 6;     // MUST BE EVEN
      int l;
const int ls[]    = {0,1,2,3,4,5};     // l=L is fully packed
...
int prints = 400;            // simulation time
...
void print_N_spins() {
	int N_zeros, N_ones, N_threes;
	std::ofstream myfile_zeros;
	std::ofstream myfile_ones;
	std::ofstream myfile_twos;
	std::ofstream myfile_threes;
	std::string filename_zeros  = "../" + type + "/N_zeros_";
	std::string filename_ones   = "../" + type + "/N_ones_";
	std::string filename_twos   = "../" + type + "/N_twos_";
	std::string filename_threes = "../" + type + "/N_threes_";

	filename_zeros += "L" + std::to_string(L) + "l" + std::to_string(l) + ".dat";
	filename_ones += "L" + std::to_string(L) + "l" + std::to_string(l) + ".dat";
	filename_threes += "L" + std::to_string(L) + "l" + std::to_string(l) + ".dat";

	myfile_zeros.open(filename_zeros);
	myfile_ones.open(filename_ones);
	myfile_threes.open(filename_threes);
	myfile_zeros << "Number of zeros in a" + std::to_string(width) + "by"
			+ std::to_string(height) + "system. " + std::to_string(steps_per_print) + "time per step.\n";
	myfile_ones << "Number of ones in a" + std::to_string(width) + "by"
			+ std::to_string(height) + "system. " + std::to_string(steps_per_print) + "time per step.\n";
	myfile_threes << "Number of threes in a" + std::to_string(width) + "by"
			+ std::to_string(height) + "system. " + std::to_string(steps_per_print) + "time per step.\n";
	...
	for (run = 0; run < runs; run++) {
		...
		for (step = 0; step < prints; step++) {
			update_func(steps_per_print);
			N_zeros = N_ones = N_threes = 0;
			for (auto spin : underlying_spins) {
				switch(spin) {
					case 0: 
						N_zeros++;
						break;
			...
			myfile_zeros << ", " << N_zeros;
			myfile_ones << ", " << N_ones;
			myfile_threes << ", " << N_threes;
		}
		myfile_zeros << "\n";
		myfile_ones << "\n";
		myfile_threes << "\n";
